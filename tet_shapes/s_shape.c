...
/* S tetromino. C++ reference: shapes/s_shape.h SShape::create_shape /
 * update_shape. */
static void s_create(Shape *s, int8_t x, int8_t y) {
...
static void s_update(Shape *s, int8_t rotate) {
    if (s->rotation == 0) {
        if (rotate == 1) {
            /* 0 -> 1 */
            s->blocks[0].y += 2;
...
static const ShapeVTable S_VT = {
    .update_shape = s_update,
    .create_shape = s_create,
};

void s_shape_init(Shape *s, int8_t x, int8_t y, int8_t rotation) {
    shape_init(s, &S_VT, x, y, rotation);
}
