...
/* J tetromino: 1-tall column at rotation 0 with foot to the left.
 * C++ reference: shapes/j_shape.h JShape::create_shape / update_shape. */
static void j_create(Shape *s, int8_t x, int8_t y) {
...
static void j_update(Shape *s, int8_t rotate) {
    if (s->rotation == 0) {
        if (rotate == 1) {
            /* 0 -> 1 */
            s->blocks[0].y += 1;
...
static const ShapeVTable J_VT = {
    .update_shape = j_update,
    .create_shape = j_create,
};

void j_shape_init(Shape *s, int8_t x, int8_t y, int8_t rotation) {
    shape_init(s, &J_VT, x, y, rotation);
}
